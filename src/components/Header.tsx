  ...
  return (
    <header
      className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 transition-all duration-500 w-[95%] max-w-7xl rounded-2xl border ${isScrolled
          ? 'bg-white/40 backdrop-blur-xl border-white/20 shadow-2xl py-3'
    ...
    >
      <nav className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center space-x-1">
          ...
          {/* Perbarui target di sini: scrollToSection('clients') */}
          <button
        ...
        {/* Mobile Toggle */}
        <button
          className={`md:hidden p-2 rounded-lg transition-colors ${isScrolled ? 'bg-gray-100 text-gray-700' : 'bg-white/10 text-white'
            }`}
